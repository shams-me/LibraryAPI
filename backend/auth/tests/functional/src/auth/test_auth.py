...
from permissions.enums import ServiceInternalSrc
...
async def test_signin_user_valid(
    ...
    user_permission_1 = users_models.UserPermissions(
...
@pytest.mark.parametrize(
    "sign_in_events, user_agent", [(1, "firefox/1"), (3, "chrome/2.50"), (1, ""), (3, "")]
)
async def test_signin_user_valid_signin_is_saved(
    ...
    stmt = select(users_models.UserSigninHistory).where(users_models.UserSigninHistory.user_id == user_0.id)
...
async def test_signup_user_exists(
    ...
    response = await client.post("/api/v1/auth/signup", headers=headers, content=orjson.dumps(content))
    assert response.status_code == expected_status
...
async def test_signup_user_invalid_data(
    client: AsyncClient, headers: dict[str, str], content: dict[str, str], expected_status: int
) -> None:
    response = await client.post(
        "/api/v1/auth/signup",
        headers=headers,
...
async def test_signout_user(
    ...
    user_permission_1 = users_models.UserPermissions(permission_id=permission_1.id, user_id=user_1.id)
...
@pytest.mark.parametrize(
    "signin_credentials, expected_status",
    [
        ("username=superuser&password=Ab1234567!", HTTPStatus.OK),
        ("username=regular_user&password=$ecretAb1", HTTPStatus.OK),
    ],
)
async def test_verify_token_invalid_revoked(
    ...
    await flushable_redis_client.set(
        name=access_jti,
        value="",
    )
    await flushable_redis_client.set(
