...
async def test_change_password(db_session: AsyncSession, client: AsyncClient) -> None:
    permission = permissions_models.Permission(
        id=UUID("743a1c54-aa4c-4576-bf77-b9a0ed7d819b"),
        src=ServiceInternalSrc.users,
        type=ServiceInternalPermission.update,
    )
    user = users_models.User(
        id=UUID("5b90edb4-ac98-4053-ac8b-93203aa8a039"),
        username="superuser",
    ...
    user_permission = users_models.UserPermissions(permission_id=permission.id, user_id=user.id)
    db_session.add_all([user_permission])
    await db_session.commit()
...
async def test_assign_permission(db_session: AsyncSession, client: AsyncClient) -> None:
    ...
    user_permission = users_models.UserPermissions(permission_id=permission_update.id, user_id=user.id)
    db_session.add_all([user_permission])
    await db_session.commit()
    ...
    assert user_data["permissions"] == {"users": ["READ", "UPDATE"]}
...
async def test_revoke_permission(db_session: AsyncSession, client: AsyncClient) -> None:
    ...
    user_permission_update = users_models.UserPermissions(permission_id=permission_update.id, user_id=user.id)
    user_permission_read = users_models.UserPermissions(permission_id=permission_read.id, user_id=user.id)
    ...
    assert user_data["permissions"] == {"users": ["UPDATE"]}
...
async def test_update_user(db_session: AsyncSession, client: AsyncClient) -> None:
    ...
    user_permission_update = users_models.UserPermissions(
