...
from src.auth.jwt.backend import (
    JWTAuthorizationBackend,
)
from src.auth.jwt.exceptions import (
    InvalidJWTError,
)
from src.auth.jwt.service import (
    validate_jwt,
)
from src.auth.jwt.storage import (
    JWTStorage,
)
...
class UserSignUp(BaseModel):
    ...
    password: str = Field(
        min_length=users_settings.password_min_length, max_length=users_settings.password_max_length
    )
    first_name: str = Field(
        min_length=users_settings.name_min_length,
        max_length=users_settings.name_max_length,
        pattern=users_settings.name_pattern,
    )
    last_name: str = Field(
        min_length=users_settings.name_min_length,
        max_length=users_settings.name_max_length,
...
class AccessToken(BaseModel):
    token_string: str = Field(pattern=r"^(?:[\w-]*\.){2}[\w-]*$")
...
async def get_auth_backend(redis: Annotated[Redis, Depends(get_redis)]) -> JWTAuthorizationBackend:
    """Initialize JWT authorization backend that uses Redis cache as a storage."""
    return JWTAuthorizationBackend(jwt_storage=JWTStorage(cache=RedisCache(redis_client=redis)))
...
async def handle_access_token(
    ...
    if not access_token_data.type == "access":
        logger.error(
            "Token type is %s, must be 'access'",
            access_token_data.type,
        )
        raise InvalidAccessTokenError

    is_jwt_active = await auth_backend.verify_jwt_credentials_are_active(decoded_token=access_token_data)
...
async def handle_refresh_token(
    ...
    if not refresh_token_data.type == "refresh":
        logger.error(
            "Token type is %s, must be 'refresh'",
            refresh_token_data.type,
        )
        raise InvalidRefreshTokenError

    is_jwt_active = await auth_backend.verify_jwt_credentials_are_active(decoded_token=refresh_token_data)
    if not is_jwt_active:
        logger.error("Token is revoked (not active)")
        raise InvalidRefreshTokenError
...
def get_user_fingerprint(request: Request) -> UserFingerprint:
    ...
    if client := request.client:
        ip = client.host
    return UserFingerprint(ip_address=ip, user_agent=request.headers.get("user-agent", ""))
