...
from src.auth.jwt import (
    schemas as jwt_schemas,
)
from src.auth.jwt.backend import JWTAuthorizationBackend
from src.role.models import RolePermissions  # noqa401, noqa403
from src.users import schemas as users_schemas
from src.users.exceptions import UserDoesNotExistError, UserPermissionError, UserUpdateError
...
class IUserService(ABC):
    ...
    @abstractmethod
    async def get_user_history(self, user_id: UUID) -> Page[users_schemas.UserLoginRecord]:
        ...
    ...
    @abstractmethod
    async def assign_user_permission(self, user_id: UUID, permission_id: UUID) -> users_schemas.User:
        ...

    @abstractmethod
    async def assign_user_permissions(self, user_id: UUID, permission_ids: list[UUID]) -> users_schemas.User:
        ...

    @abstractmethod
    async def revoke_user_permission(self, user_id: UUID, permission_id: UUID) -> users_schemas.User:
        ...

    @abstractmethod
    async def assign_user_role(self, user_id: UUID, role_id: UUID) -> users_schemas.User:
        ...

    @abstractmethod
    async def revoke_user_role(self, user_id: UUID, role_id: UUID) -> users_schemas.User:
        ...
...
class UserService(IUserService):
    ...
    async def signin(
        ...
        if user_db := await self.get_by_username(user.username):
            ...
            if is_password_ok:
                ...
                logger.debug("User signin has been saved: %s", str(sign_in_record.model_dump()))
    ...
    async def change_user_password(
        ...
        if not await self.user_repository.check_password(user_id=user_id, password=old_password):
            raise UserInvalidCredentialsError
    ...
    async def assign_user_permission(self, user_id: UUID, permission_id: UUID) -> users_schemas.User:
        ...
        try:
            return await self.user_repository.assign_permission(
                user_id,
                permission_id,
            )
    ...
    async def assign_user_role(self, user_id: UUID, role_id: UUID) -> users_schemas.User:
        ...
        try:
            return await self.user_repository.assign_role(user_id, role_id)
        except sqlalchemy.exc.IntegrityError:
            raise UserPermissionError
    ...
    async def revoke_user_role(self, user_id: UUID, role_id: UUID) -> users_schemas.User:
        ...
        try:
            return await self.user_repository.revoke_role(user_id, role_id)
        except sqlalchemy.exc.IntegrityError:
            raise UserPermissionError
    ...
    async def update_user_common_info(
        ...
        try:
            return await self.user_repository.update(user_id, username, first_name, last_name)
        except sqlalchemy.exc.IntegrityError:
