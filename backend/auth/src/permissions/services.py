...
from src.permissions.exceptions import PermissionDoesNotExistError, PermissionNameExistsError
from src.permissions.repositories import PermissionRepository, PostgresPermissionRepository


class IPermissionsService(ABC):
    @abstractmethod
    async def create_permission(
        self, permission: permissions_schemas.PermissionCreate
    ) -> permissions_schemas.Permission:
        ...

    @abstractmethod
    async def delete_permission(self, permission_id: UUID) -> permissions_schemas.Permission:
        ...

    @abstractmethod
    async def update_permission(
        self, permission_id: UUID, permission: permissions_schemas.PermissionUpdate
    ) -> permissions_schemas.Permission:
        ...
    ...
    @abstractmethod
    async def get_permission_by_name(self, permission_name: str) -> permissions_schemas.Permission | None:
        ...
...
class PermissionsService(IPermissionsService):
    ...
    async def create_permission(
        ...
        try:
            permission_db = await self.repo.create(**permission.model_dump(exclude_none=True))
    ...
    async def update_permission(
        self,
        permission_id: UUID,
        permission: permissions_schemas.PermissionUpdate,
    ) -> permissions_schemas.Permission:
        if permission_db := await self.repo.get(permission_id):
            return await self.repo.update(
                permission_id=permission_db.id,
                **permission.model_dump(exclude_none=True),
            )
        raise PermissionDoesNotExistError
    ...
    async def get_permission_by_name(self, permission_name: str) -> permissions_schemas.Permission | None:
        return await self.repo.get_by_src(permission_name)
    ...
    async def create_internal_permissions(self) -> list[permissions_schemas.Permission]:
        ...
        existing_permissions_names = [permission.src for permission in existing_permissions]
        ...
        created_permissions = await self.repo.create_bulk(
            names=permissions_names_to_create,
            type="internal",
        )
...
async def get_permissions_service(db_session: AsyncSession = Depends(get_db)) -> IPermissionsService:
    return PermissionsService(permissions_repo=PostgresPermissionRepository(async_session=db_session))
