...
from src.cli.superuser import (
    create_user_with_internal_permissions,
)
...
def create_superuser(
    ...
    try:
        (created_user, created_role_name) = asyncio.run(
        ...
        typer.echo(user_exists_msg.format(username=validated_user.username), err=True)
        typer.echo(aborting_msg, err=True)
        raise typer.Exit(code=1)

    success_msg = typer.style(msg.SUPERUSER_CREATED, fg=typer.colors.GREEN, bold=True)
    success_permissions_msg = typer.style(msg.PERMISSIONS_CREATED, fg=typer.colors.GREEN, bold=True)
    typer.echo(success_msg.format(user_data=created_user.model_dump(exclude={"password", "permissions"})))
    typer.echo(success_permissions_msg.format(permissions=created_role_name))
